...
// buttons, so nothing here starts an OAuth redirect.
//
// The Decane SDK (decane-connect-kit) is an embedded-WALLET kit: it goes on to
// create a crypto wallet and demands a passkey or PIN to encrypt the wallet's
// key share. Vivid needs none of that, and Decane issues the token before any
// wallet step, so this calls the two relevant endpoints directly and never
// loads the SDK.
//
